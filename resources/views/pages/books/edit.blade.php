...
@section('content')
<div class="">
    <x-common.page-breadcrumb pageTitle="Edit Book" />
    ...
    <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            <!-- Same fields as create form but with values from $book -->
            <x-common.component-card title="Basic Information" desc="Edit the basic details of the book">
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="series" value="{{ old('series', $book->series) }}"
            ...
            <!-- Book Details (similar to create) -->
            <x-common.component-card title="Book Details" desc="Edit additional information about the book">
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            ISBN
                    ...
                    <div>
                        ...
                        <x-form.date-picker 
                            ...
                            defaultDate="{{ old('publish_date', $book->publish_date?->format('Y-m-d')) }}" 
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" name="price" value="{{ old('price', $book->price) }}"
                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-4 pr-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('price') border-error-300 @enderror">
                        </div>
                        @error('price')
                            <p class="text-theme-xs text-error-500 mt-1.5">{{ $message }}</p>
                        @enderror
            ...
            <!-- Genres and Characters with pre-selected values -->
            <x-common.component-card title="Categories" desc="Edit genres and characters for this book">
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <!-- Genres -->
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        ...
                        <div x-data="{
                            open: false,
                            selected: {{ json_encode($book->genres->pluck('id')->toArray()) }},
                            ...
                            <!-- Same dropdown HTML as create form -->
                            <div @click="open = !open"
                    ...
                    <!-- Characters with pre-selected values -->
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            Characters
                        </label>
                        <div x-data="{
                            open: false,
                            selected: {{ json_encode($book->characters->pluck('id')->toArray()) }},
            ...
            <!-- Cover Image with current image preview -->
            <x-common.component-card title="Book Cover" desc="Upload a new cover image (optional)">
                @if($book->cover_img)
                    <div class="mb-4">
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Current Cover</label>
                        <img src="{{ Storage::url($book->cover_img) }}" alt="{{ $book->title }}" class="h-32 w-auto rounded-lg object-cover">
                    </div>
                @endif
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.books.index') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700">Cancel</a>
                <button type="submit" class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">Update Book</button>
            </div>
        </div>
    </form>
</div>
